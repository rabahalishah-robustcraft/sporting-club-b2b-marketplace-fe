...
export const ClubDashboard = () => {
  ...
  const handleViewOpportunities = (businessData) => {
...
const MainContent = ({ onViewOpportunities }) => {
  ...
  return (
    <div className="flex flex-col space-y-6 flex-1">
      ...
      <div className="p-2">
        <h1 className="text-2xl font-bold">Find Local Business</h1>
        <p className="text-gray-600 mt-2">
          Browse local businesses ready to partner with your club.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredClubs.length > 0 ? (
          filteredClubs.map((business, index) => (
            <BusinessCard
              key={index}
              businessName={business.businessName}
              sponsorships={business.sponsorships}
              location={`${business.city} • ${business.distance}`}
              listedDate={business.listedDate}
              onViewOpportunities={() => onViewOpportunities(business)}
            />
          ))
        ) : (
