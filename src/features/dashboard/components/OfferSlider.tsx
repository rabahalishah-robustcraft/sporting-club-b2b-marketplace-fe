...
export const OfferSlider = ({ isOpen, onClose, business }) => {
  ...
  // Don't render if the slider is not open or no business is selected
  if (!isOpen || !business) {
    return null;
  }
  ...
  return (
    ...
    >
      ...
      <div className="text-center mt-4">
        <h3 className="text-3xl font-bold">{business.businessName}</h3>
      ...
      <div className="flex flex-wrap justify-center items-center">
        <img
          ...
          className="h-7"
        />
        {business.sponsorships.map((sponsorship, index) => (
          <span
            key={index}
      ...
      <div className="flex flex-wrap text-black text-sm space-x-2">
        <div className="flex items-center space-x-2">
          <span>{`${business?.numOfCompletedOrders} Orders Completed`}</span>
        </div>
        <div className="flex items-center space-x-2 text-black">
          <span>&#8226; {business?.listedDate}</span>
        </div>
      </div>
